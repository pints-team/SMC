...
class StochasticModelTauLeaping:
    def __init__(self, x_0, events, step_size, R):
        ...
        self.states = [x_0]
        self.measurements = [x_0 + np.random.multivariate_normal(mean=[0] * len(self.R), cov=self.R)]
    ...
    def measure(self, t):
        if t < len(self.measurements):
            ...
            for _ in range(diff):
                ...
                self.states.append(new_state)
                new_measurement = new_state + np.random.multivariate_normal(mean=[0] * len(self.R), cov=self.R)
                self.measurements.append(new_measurement)
            return self.measure(t)
